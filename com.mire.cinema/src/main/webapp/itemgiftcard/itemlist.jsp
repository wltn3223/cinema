				...
				<div id="tab01">
					...
					<div class="item-container">
						<c:forEach var="item" items="${itemGiftCards}">
							<a href="/itemgiftcard/itemEditForm">
								<div class="item">
									...
									<p style="color: #9d00f7; font-size: 25px; font-weight: 400;">${item.price}원</p>
								</div>
							</a>
						</c:forEach>
					</div>
	...
	<script>
		...
		function appendItemGiftCardToTable(itemGiftCards) {
		    ...
		    for (var i = 0; i < itemGiftCards.length; i++) {
		        ...
		        var row = '<a href="/itemgiftcard/itemEditForm.jsp">' +
		            '<div class="item">' +
		            '<img src="' + itemGiftCard.image + '" alt="' + itemGiftCard.itemName + ' 이미지" width="220" height="220">' +
		            '<p style="color: black; font-weight: bold;">' + itemGiftCard.itemName + '</p>' +
		            '<p style="color: black; font-size: 13px;">' + itemGiftCard.itemInfo + '</p>' +
		            '<p style="color: #9d00f7; font-size: 25px; font-weight: 400;">' + itemGiftCard.itemPrice + '원</p>' +
		            '</div>' +
		            '</a>';
		        div.append(row);
		    }
